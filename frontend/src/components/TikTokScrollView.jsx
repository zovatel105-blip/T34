...
// ---------------------------------------------------------------------------
// useVideoPreDecode  ← TikTok's secret sauce
// Forces the browser to fully decode the first frame into GPU memory so
// the transition is zero-latency.
// ---------------------------------------------------------------------------
function useVideoPreDecode(url, enabled) {
  const videoRef = useRef(null);

  useEffect(() => {
    if (!enabled || !url) return;

    // Create an off-screen video element and force decode
    ...
    // Play → immediately pause: forces GPU decode of first keyframe
    video.play()
      .then(() => { video.pause(); videoRef.current = video; })
      .catch(() => { cleanup(); });
...
// ---------------------------------------------------------------------------
// TikTokPollCard  (unchanged business logic, added pre-decode awareness)
// ---------------------------------------------------------------------------
const TikTokPollCard = ({
  poll,
  onVote, onLike, onShare, onComment, onSave, onCreatePoll,
  isActive, index, total, showLogo = true,
  shouldPreload = true, isVisible = true,
  onUpdatePoll, onDeletePoll, isOwnProfile,
  currentUser: authUser, savedPolls, setSavedPolls,
  commentedPolls, setCommentedPolls, sharedPolls, setSharedPolls,
  optimizeVideo = false, renderPriority = 'medium', shouldUnload = false,
  layout = null, distanceFromActive = 0, isHighBandwidth = true,
  onModalStateChange = null, showScrollHint = false,
  ...
  const handleTouchEnd = (e) => {
    ...
    if (diff > 0) setCurrentSlide(prev => Math.min(prev + 1, (poll.options?.length || 1) - 1));
    else setCurrentSlide(prev => Math.max(prev - 1, 0));
    setTouchStart(null);
  };

  const handleCarouselThumbnailChange = (thumbnailUrl) => setCarouselThumbnail(thumbnailUrl);
  const handleCarouselAudioChange = (audioData) => {
    setCarouselAudioId(audioData?.id || null);
    setCarouselAudioData(audioData);
  };
  ...
  useEffect(() => {
    if (!isActive && showCommentsModal) setShowCommentsModal(false);
    if (!isActive && showPostDetailModal) setShowPostDetailModal(false);
  ...
  useEffect(() => {
    ...
    if (onModalStateChange) onModalStateChange(isAnyModalOpen);
  ...
  const handleNotInterested = async (pollId) => {
    try { await feedMenuService.markNotInterested(pollId); return { success: true }; }
    catch (error) { console.error('Error marking as not interested:', error); throw error; }
  };
  const handleHideUser = async (authorId) => {
    try { await feedMenuService.hideUser(authorId); return { success: true }; }
    catch (error) { console.error('Error hiding user:', error); throw error; }
  };
  const handleToggleNotifications = async (authorId) => {
    try {
      ...
      return { success: true };
    } catch (error) { console.error('Error toggling notifications:', error); throw error; }
  };
  const handleReport = async (pollId, reportData) => {
    try { await feedMenuService.reportContent(pollId, reportData); return { success: true }; }
    catch (error) { console.error('Error submitting report:', error); throw error; }
  };
  ...
  const handleStoryViewerClose = async () => {
    ...
    } catch (error) { console.error('Error reloading author stories:', error); }
  };
  ...
  // ─── AUDIO SYNC ─────────────────────────────────────────────────────────
  // NOTE: Audio is now managed EXCLUSIVELY at TikTokScrollView level.
  // This card only activates AudioContext on first interaction.
  useEffect(() => {
  ...
  const handleVote = (optionId) => {
    if (!poll.userVote) onVote(poll.id, optionId);
  };
  ...
  const handleFollowUser = async (user) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast({ title: "Error", description: result.error || "Error al seguir al usuario", variant: "destructive", duration: AppConfig.TOAST_DURATION });
      }
    } catch (error) {
    ...
    <div className="w-full h-full flex flex-col relative bg-black overflow-hidden">
      <div
        ...
        style={isPostMiniature ? {
          top: '8px', left: 0, right: 0, bottom: 'auto',
          width: '100%', height: '59vh', borderRadius: '20px',
          overflow: 'hidden', zIndex: 101, pointerEvents: 'none',
      ...
      >
              ...
              {poll.is_challenge && poll.participants && poll.participants.length > 0 ? (
                <button className="flex items-center gap-2" onClick={(e) => { e.stopPropagation(); setShowChallengeParticipants(true); }}>
                  <div className="relative flex-shrink-0" style={{ width: '48px', height: '48px' }}>
                    {poll.participants[1] && (
                      <div className="rounded-full overflow-hidden bg-white absolute" style={{ zIndex: 1, top: '0px', right: '0px', width: '30px', height: '30px', WebkitMaskImage: 'radial-gradient(circle at 0px 30px, transparent 19px, black 20px)', maskImage: 'radial-gradient(circle at 0px 30px, transparent 19px, black 20px)' }}>
                        {poll.participants[1].avatar_url
                          ? <img src={resolveAssetUrl(poll.participants[1].avatar_url)} alt={poll.participants[1].username || ''} className="w-full h-full object-cover" />
                          : <div className="w-full h-full flex items-center justify-center bg-gray-50 text-gray-400"><User className="w-3 h-3" /></div>}
                      </div>
                    )}
                    {poll.participants[0] && (
                      <div className="rounded-full overflow-hidden bg-white absolute" style={{ zIndex: 2, bottom: '0px', left: '0px', width: '36px', height: '36px' }}>
                        {poll.participants[0].avatar_url
                          ? <img src={resolveAssetUrl(poll.participants[0].avatar_url)} alt={poll.participants[0].username || ''} className="w-full h-full object-cover" />
                          : <div className="w-full h-full flex items-center justify-center bg-gray-50 text-gray-400"><User className="w-4 h-4" /></div>}
                      </div>
                    )}
                  </div>
                  <div>
                    <span className="text-white text-sm font-bold leading-tight drop-shadow-md">
                      {(() => {
                        ...
                        if (total === 2) { const p2 = poll.participants[1]; const name2 = p2.username || p2.display_name || 'usuario'; return `${name1} vs ${name2}`; }
                        const rest = total - 1;
                ...
                <>
                  <div className="group relative">
                    <button onClick={handleAvatarClick} className="w-12 h-12 rounded-full relative">
                      ...
                      <div className="absolute rounded-full overflow-hidden" style={{ inset: '3.5px' }}>
                        <Avatar className="w-full h-full rounded-full">
                          <AvatarImage src={poll.author?.avatar_url && poll.author.avatar_url !== null ? resolveAssetUrl(poll.author.avatar_url) : undefined} className="object-cover" />
                    ...
                    {!isFollowing(authorUserId) && currentUser && authorUserId !== currentUser.id && (
                      <button
                        onClick={(e) => { e.stopPropagation(); const userToFollow = poll.authorUser || { username: (poll.author?.username || poll.author?.display_name || 'unknown').toLowerCase().replace(/\s+/g, '_'), displayName: poll.author?.display_name || poll.author?.username || 'Usuario', id: authorUserId }; handleFollowUser(userToFollow); }}
                    ...
                    {isFollowing(authorUserId) && (
                      <div className="absolute bottom-1 right-1 bg-white rounded-full p-[2px] shadow-lg" style={{ animation: 'followBounce 0.5s cubic-bezier(0.175, 0.885, 0.32, 1.275) forwards' }}>
            ...
            <div className="w-full h-full flex flex-col">
              <div className="flex-1 flex">
                {(() => {
                  ...
                  const getP = (votes) => totalVotes === 0 ? 0 : Math.round((votes / totalVotes) * 100);
                    ...
                    return (
                      <>
                        {poll.options.map((option, optIdx) => {
                          const percentage = getP(option.votes || 0);
                          ...
                          return (
                            <div key={option.id || optIdx} className={cn("flex-1 relative overflow-hidden", optIdx === 0 ? "border-r-2 border-white/30" : "")}>
                              <DoubleTapVoteAnimation onDoubleTap={() => !hasUserVoted && handleVote(option.id)} disabled={hasUserVoted}>
                                <PollOptionMedia option={option} className="w-full h-full" videoProps={{ autoPlay: true, loop: true, muted: true, playsInline: true }} />
                                {option.mentioned_users?.length > 0 && (
                                  <div className="absolute bottom-16 left-2 right-2 z-20 flex flex-wrap gap-1 items-center">
                                    {option.mentioned_users.slice(0, 2).map((mentionedUser, mIdx) => (
                                      <button key={mentionedUser.id || mIdx} onClick={(e) => { e.stopPropagation(); const username = mentionedUser.username || mentionedUser.display_name?.toLowerCase().replace(/\s+/g, '_'); if (username) navigate(`/profile/${username}`); }} className="flex items-center bg-white/20 px-1.5 py-0.5 rounded-full backdrop-blur-sm cursor-pointer hover:bg-white/30 transition-all duration-200">
                                        <Avatar className="w-4 h-4 mr-1 border border-white/50"><AvatarImage src={resolveAssetUrl(mentionedUser.avatar_url)} /><AvatarFallback className="bg-gray-400 text-white text-[8px] flex items-center justify-center"><User className="w-2 h-2" /></AvatarFallback></Avatar>
                                        <span className="text-[10px] text-white font-medium">@{(mentionedUser.username || mentionedUser.display_name)?.slice(0, 10)}</span>
                                      </button>
                                    ))}
                              ...
                              <DoubleTapVoteAnimation onDoubleTap={() => !hasUserVoted && handleVote(option.id)} disabled={hasUserVoted}>
                                ...
                                {option.mentioned_users?.length > 0 && (
                                  <div className="absolute bottom-12 left-1 right-1 z-20 flex flex-wrap gap-0.5 items-center">
                                    {option.mentioned_users.slice(0, 2).map((mentionedUser, mIdx) => (
                                      <button key={mentionedUser.id || mIdx} onClick={(e) => { e.stopPropagation(); const username = mentionedUser.username || mentionedUser.display_name?.toLowerCase().replace(/\s+/g, '_'); if (username) navigate(`/profile/${username}`); }} className="flex items-center bg-white/20 px-1 py-0.5 rounded-full backdrop-blur-sm cursor-pointer hover:bg-white/30 transition-all duration-200">
                                        <Avatar className="w-3 h-3 mr-0.5 border border-white/50"><AvatarImage src={resolveAssetUrl(mentionedUser.avatar_url)} /><AvatarFallback className="bg-gray-400 text-white text-[7px] flex items-center justify-center"><User className="w-2 h-2" /></AvatarFallback></Avatar>
                                        <span className="text-[9px] text-white font-medium">@{(mentionedUser.username || mentionedUser.display_name)?.slice(0, 8)}</span>
                                      </button>
                                    ))}
        ...
        <div className="absolute bottom-0 left-0 right-0 z-30 bg-gradient-to-t from-black/90 via-black/70 to-transparent px-4 pt-8 pointer-events-none"
          style={{ paddingBottom: isPostMiniature ? 'max(0.5rem, var(--safe-area-inset-bottom))' : (isBottomNavVisible ? 'calc(56px + max(0.5rem, var(--safe-area-inset-bottom)))' : 'max(1.5rem, var(--safe-area-inset-bottom))'), paddingLeft: 'max(1rem, var(--safe-area-inset-left))', paddingRight: 'max(1rem, var(--safe-area-inset-right))' }}>
          {(poll.show_vote_count !== false && poll.showVoteCount !== false) && (
            <div className="mb-4 pointer-events-auto">
              {poll.is_challenge ? (
                ...
                <button onClick={(e) => { e.stopPropagation(); setShowVotersModal(true); }} className="inline-flex items-center px-4 py-2 rounded-full bg-black/20 backdrop-blur-sm text-white/90 font-semibold text-sm hover:text-white transition-colors cursor-pointer">
          ...
          {(() => {
            ...
            return (
              <div className={`flex items-center -ml-2 pointer-events-auto flex-nowrap ${hasHighInteractions ? 'gap-0.5' : 'gap-3'}`}>
                <Button variant="ghost" size="sm" onClick={(e) => { e.stopPropagation(); onLike(poll.id); }} className={cn(`flex items-center gap-1 hover:scale-105 transition-all duration-200 text-white hover:text-red-400 h-auto p-1.5 rounded-lg bg-black/20 backdrop-blur-sm`, poll.userLiked && "text-red-500 bg-red-500/20")}>
                  <Heart className={cn("w-5 h-5 flex-shrink-0 transition-all duration-200", poll.userLiked && "fill-current scale-110")} />
                  <span className="font-medium text-sm whitespace-nowrap">{formatNumber(poll.likes)}</span>
                </Button>
                <Button variant="ghost" size="sm"
                  onClick={(e) => { e.stopPropagation(); setShowCommentsModal(true); const commentsEnabled = poll.comments_enabled !== false && poll.commentsEnabled !== false; if (commentsEnabled) { setCommentedPolls(prev => { const newSet = new Set(prev); newSet.add(poll.id); return newSet; }); } }}
                  className={`flex items-center gap-1 hover:scale-105 transition-all duration-200 h-auto p-1.5 rounded-lg backdrop-blur-sm ${commentedPolls.has(poll.id) || poll.userCommented ? 'text-blue-400 bg-blue-500/20 hover:text-blue-300' : 'text-white bg-black/20 hover:text-blue-400'}`}>
                  <MessageCircle className={`w-5 h-5 flex-shrink-0 ${commentedPolls.has(poll.id) || poll.userCommented ? 'fill-current' : ''}`} />
                  {(poll.comments_enabled !== false && poll.commentsEnabled !== false) && <span className="font-medium text-sm whitespace-nowrap">{formatNumber(poll.comments)}</span>}
                </Button>
                <Button variant="ghost" size="sm"
                  onClick={async (e) => {
                    e.stopPropagation();
                    const registerShareInBackend = async () => {
                      ...
                      if (token) {
                        try {
                          const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/polls/${poll.id}/share`, { method: 'POST', headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' } });
                          if (response.ok) { setSharedPolls(prev => { const newSet = new Set(prev); newSet.add(poll.id); return newSet; }); }
                        } catch (error) { console.error('Error sharing poll:', error); }
                      }
                    };
                    if (navigator.share) {
                      navigator.share({ title: poll.question || 'Vota en esta encuesta', text: 'Mira esta increíble votación', url: `${window.location.origin}/poll/${poll.id}` })
                        .then(async () => { await registerShareInBackend(); onShare && onShare(poll.id); })
                        .catch((error) => { if (error.name !== 'AbortError') sharePoll(poll); });
                    } else { sharePoll(poll); }
                  }}
                  className={`flex items-center gap-1 hover:scale-105 transition-all duration-200 h-auto p-1.5 rounded-lg backdrop-blur-sm ${sharedPolls.has(poll.id) || poll.userShared ? 'text-green-400 bg-green-500/20 hover:text-green-300' : 'text-white bg-black/20 hover:text-green-400'}`}>
                ...
                {onSave && (
                  <Button variant="ghost" size="sm"
                    onClick={async (e) => {
                      ...
                      try {
                        if (isCurrentlySaved) {
                          const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/polls/${poll.id}/save`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}`, 'Content-Type': 'application/json' } });
                          if (response.ok) setSavedPolls(prev => { const newSet = new Set(prev); newSet.delete(poll.id); return newSet; });
                        } else {
                          onSave(poll.id);
                          setSavedPolls(prev => { const newSet = new Set(prev); newSet.add(poll.id); return newSet; });
                        }
                      } catch (error) { console.error('Error with save/unsave:', error); }
                    }}
                    className={`flex items-center gap-1 hover:scale-105 transition-all duration-200 h-auto p-1.5 rounded-lg backdrop-blur-sm ${savedPolls.has(poll.id) || poll.isSaved ? 'text-yellow-400 bg-yellow-500/20 hover:text-yellow-300' : 'text-white bg-black/20 hover:text-yellow-400'}`}>
                    <Bookmark className={`w-5 h-5 flex-shrink-0 ${savedPolls.has(poll.id) || poll.isSaved ? 'fill-current' : ''}`} />
                    {(poll.saves_count || 0) > 0 && <span className="font-medium text-sm whitespace-nowrap">{formatNumber(poll.saves_count)}</span>}
                  </Button>
                )}
                <Button variant="ghost" size="sm"
                  onClick={(e) => { e.stopPropagation(); setIsMenuOpen(true); }}
                  className="flex items-center gap-1 hover:scale-105 transition-all duration-200 text-white hover:text-white h-auto p-1.5 rounded-lg bg-black/20 backdrop-blur-sm ml-auto">
          ...
          {/* Music display */}
          {(poll.music || carouselAudioData) && (
            <div className="mt-3 pointer-events-auto">
              <MusicDisplay
                music={carouselAudioData || poll.music}
        ...
        {showScrollHint && (
          <div className="absolute bottom-32 left-1/2 transform -translate-x-1/2 flex flex-col items-center gap-3 z-20"
            style={{ bottom: 'max(8rem, calc(8rem + var(--safe-area-inset-bottom)))' }}>
            <div className="animate-bounce"><ChevronDown className="w-8 h-8 text-white/80" /></div>
            <div className="text-white/80 text-sm font-medium bg-black/60 px-4 py-2 rounded-full backdrop-blur-sm border border-white/20">Desliza para ver más</div>
          </div>
        )}
      </div>
      ...
      <PostDetailModal isOpen={showPostDetailModal} onClose={() => setShowPostDetailModal(false)} poll={poll} isFollowing={isFollowing(authorUserId) || (currentUser && authorUserId === currentUser.id)} onFollow={() => { const userToFollow = poll.authorUser || { username: (poll.author?.username || poll.author?.display_name || 'unknown').toLowerCase().replace(/\s+/g, '_'), displayName: poll.author?.display_name || poll.author?.username || 'Usuario', id: authorUserId }; handleFollowUser(userToFollow); }} commentsEnabled={poll.comments_enabled !== false && poll.commentsEnabled !== false} />
      <VotersModal isOpen={showVotersModal} onClose={() => { setShowVotersModal(false); setIsVotersExpanded(false); }} pollId={poll.id} onExpandChange={setIsVotersExpanded} />
      {poll.is_challenge && <ChallengeParticipantsModal isOpen={showChallengeParticipants} onClose={() => setShowChallengeParticipants(false)} participants={poll.participants || []} challengeTitle={poll.title} />}
      <ShareModal isOpen={shareModal.isOpen} onClose={closeShareModal} content={shareModal.content} />
      {showAuthorStoryViewer && authorStoriesData && createPortal(
        <StoriesViewer storiesGroups={[authorStoriesData]} onClose={handleStoryViewerClose} initialUserIndex={0} />,
        document.body
      )}
      <FeedMenu
        isOpen={isMenuOpen}
