...
const TikTokPollCard = ({ 
  ...
  const renderActionButtons = (sideMode) => {
    const iconCls = sideMode ? "w-[25px] h-[25px]" : "w-5 h-5";
    const txtCls = sideMode ? "text-[12px]" : "text-sm";
    const btnLayout = sideMode
      ? "flex flex-col items-center gap-0.5 hover:scale-110 transition-all duration-200 h-auto p-0 bg-transparent hover:bg-transparent [&_svg]:size-[25px]"
              ...
              <div className="flex flex-col items-center gap-0.5 text-white" style={{ filter: 'drop-shadow(0 2px 4px rgba(0,0,0,0.7))' }}>
                {(() => {
                  ...
                  return (
                    <>
                      {showTrophy ? (
                        <Trophy className="w-[25px] h-[25px] flex-shrink-0" strokeWidth={1.5} />
                      ) : (
                        <VoteIcon className="w-[25px] h-[25px] flex-shrink-0" />
                      )}
